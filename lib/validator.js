...
var validateMember = function(typeDesc, origValue, subObjectRefs){
   ...
   for (var i = 0; i < values.length; i++){
      ...
      if (undefined !== typeDesc['@allowed_values'] && -1 === typeDesc['@allowed_values'].indexOf(value)){
         return { 'valid' : false, 'message' : '\'' + typeDesc['@property_name']
      ...
      case "int":
         ...
         break;
      case "attachment":
         if (!openiUtils.isAttachmentId(value)){
...
var validateObjectAgainstType = function(cloudletId, host, objData, typeStr, success_callback, failure_callback){
   ...
   for (var member in type['@context']){
      if(type['@context'].hasOwnProperty(member)) {
         ...
         var typeMemberType = value['@openi_type'];

         if( undefined === openiObjValue && value['@required']){
            failure_callback({'error' : 'Object does not adhere to Type: Required property ' + value['@property_name'] + " is missing." });
            return;
         }

         var memberValResult = validateMember(value, openiObjValue, subObjectRefs );
...
//               + ". "
   ...
   if (subObjectRefs.length > 0){
      ...
      validateSubObjs(cloudletId, host, obj, subObjectRefs, success_callback, failure_callback);
   }
   else{
      success_callback();
   }
...
var val_sub = function(type, obj, objs, cloudletId, host, success_callback, failure_callback){

   for (var member in type['@context']){
      if(type['@context'].hasOwnProperty(member)) {
         var value          = type['@context'][member];
         var openiObjValue  = obj[value['@property_name']];
         var typeMemberType = value['@openi_type'];

         if (( null === openiObjValue || '' === openiObjValue || undefined === openiObjValue )){
            ...
            if( value['@required']){
               ...
               var memberValResult = validateMember(value, openiObjValue, [] );
   ...
   if (0 === objs.length){
      ...
      var obj = objs.pop();
      validateSubObjs(cloudletId, host, obj, objs, success_callback, failure_callback);
   }
};


var validateSubObjs = function(cloudletId, host, obj, objs, success_callback, failure_callback){

   var typeURL = "https://" + host + "/api/v1/types/" + obj.type;

   if ('object' === typeof(obj.value)){
      //if it is literal object type we need to lookup type and validate that it adheres to values.

      getWithFunction(typeURL, objs, cloudletId, host, success_callback, failure_callback, val_sub, obj.value);
   }
   else if(openiUtils.isObjectId(obj.value)){
      //if it is a reference type we need to get it and compare the type id to value

      var objURL = "https://" + host + "/api/v1/objects/" + cloudletId + "/" + obj.value;

      getWithFunction(objURL, objs, cloudletId, host, success_callback, failure_callback, function(obj_json){

         getWithFunction(typeURL, objs, cloudletId, host, success_callback, failure_callback, val_sub, obj_json['@data']);

      });
   }
   else{
      failure_callback({'error' : 'Invalid value for type: ' + obj.type });
   }
};

var getWithFunction = function(URL, objs, cloudletId, host, success_callback, failure_callback, fun, arg){
   https.get(URL, function(res) {
      ...
      res.on('end', function () {
         ...
         fun(json, arg, objs, cloudletId, host, success_callback, failure_callback);
      });
...
var validateObject = function(cloudletId, host, typeURL, objData, success_callback, failure_callback){

   console.log(typeURL);

   if (openiUtils.isTypeId(typeURL)){
   ...
   var request = https.get(typeURL, function(res) {
      ...
      //the whole response has been recieved, so we just print it out here
      res.on('end', function () {
         validateObjectAgainstType(cloudletId, host, objData, typeStr, success_callback, failure_callback);
      });
