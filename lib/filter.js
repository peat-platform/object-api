...
var filter = function(msg, senderToClient, cloudletId){
   ...
   if (undefined === terms.with_property && undefined == terms.property_filter){
      var resp = zmq.Response(zmq.status.BAD_REQUEST_400, zmq.header_json, [])
...
//   console.log(body.filter.bool.must)
//
//   body = {
//      query: {
//      wildcard: {
...
//
   ...
   elastisearchClient.search({
      ...
      if (error) {
         ...
         for(var i = 0; i < response.hits.total; i++) {
            if (undefined !== response.hits.hits[i]){
               if (id_only){
                  ...
                  respArr[i] = {
                     '@id'       : response.hits.hits[i]._source.doc['@id'],
                     '@cloudlet' : response.hits.hits[i]._source.doc['@cloudlet']
                  }
               }
               else{
                  var filter_msg = (undefined !== terms.only_show_properties)?
                                       {'filter_show':terms.only_show_properties.split(',')} : {}
         ...
         var resp = zmq.Response(zmq.status.BAD_REQUEST_400, zmq.header_json, respArr)
