...
function formBody(obj, attr) {
   ...
   if (obj === "") {
      ...
      query = obj;
      json = JSON.parse(JSON.stringify(query));
   }
   else {
      query = [obj, attr];
      json = {
...
var filter = function (msg, meta, senderToClient, cloudletId) {
   ...
   else if (_.isUndefined(terms)) {
      body = attrQuery(attributes, op);
   }
   else {
      body = objAttrQuery(terms, attributes, op);
   }
   ...
   if (undefined !== query.type) {
      body.filter.bool["must"].push(JSON.parse("{\"term\": {\"doc.@openi_type\": \"https://" + msg.headers.host + "/api/v1/types/" + query.type + "\"}}"));
   }
