...
var genGetMessage = function(msg, senderToDao, senderToClient, terminal_handler){
   ...
   if (null === cloudletId){
      if(msg.cloudlet !== undefined) {
         cloudletId = msg.cloudletId
      }
      else {
         cloudletId = (msg.token["openi-token-type"] === "session") ? msg.token.cloudlet :  msg.token.context
      }
   }
   ...
   if(null !== objectId) {
      ...
      if (undefined !== query.type) {
         ...
         startKey = [third_party, query.type, 0,                cloudletId];
         endKey   = [third_party, query.type, Number.MAX_VALUE, cloudletId + '\uefff' ];
      }
