...
var genPostMessage = function(msg){
   var objectId = openiUtils.hash(msg.json.object)
   return {
      'dao_actions'      : [
         { 'action' : 'POST',   'database': getCloudlet(msg.path), 'object_name'  : objectId, 'object_data' : msg.json }
      ],
...
var genGetMessage = function(msg){
   return {
      'dao_actions'      : [
         { 'action' : 'GET',   'database': getCloudlet(msg.path), 'object_name'  : getObject(msg.path) }
      ],
      'mongrel_resp' : {'value':true},
      'clients'      : [
         {'uuid' : msg.uuid, 'connId' : msg.connId }
